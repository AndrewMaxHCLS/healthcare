...
def get_iam_policy_cleanup(config):
  ...
  if 'forseti' in config.root:
    forseti_service_account = field_generating.get_forseti_service_generated_fields(
...
def setup_project(config, output_yaml_path, output_cleanup_path):
  ...
  deployed = field_generating.is_deployed(project_id, config.root)
...
def install_forseti(config):
  ...
  field_generating.set_forseti_service_generated_fields(generated_field)
  ...
  def grant_access(config):
    forseti.grant_access(
        ...
        field_generating.get_generated_fields_ref(
            config.root['forseti']['project']['project_id'],
...
def main(argv):
  ...
  if field_generating.move_generated_fields_out_of_projects(FLAGS.project_yaml):
    if FLAGS.dry_run:
      logging.error(
