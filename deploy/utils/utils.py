...
def merge_two_config_dicts(root_config, config_patch, revisable):
  """Merge config_patch into root_config.

  Args:
    root_config: A config dictionary into which the other dict is merged.
    config_patch: A dict to be merged.
    revisable: True if update non-list and non-dict values.
  ...
  for key, val in config_patch.items():
    if key in root_config:
      ...
      if isinstance(val, list) and isinstance(root_value, list):
        ...
        merge_two_config_dicts(root_config[key], val, revisable)
      elif revisable and isinstance(val, type(root_value)):
        val = root_value
      else:
...
def load_config(overall_path):
  ...
  for inc_file in import_files:
    ...
    merge_two_config_dicts(overall, inc_contents, False)
